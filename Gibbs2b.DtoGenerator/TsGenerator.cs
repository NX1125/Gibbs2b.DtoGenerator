    ...
    public void Generate()
    {
        ...
        foreach (var project in _solution.TypescriptProjects)
        {
                         ...
                         .OrderBy(p => p.Name.CapitalCase))
            {
                ...
                CommitFiles();
                StartFiles(project.Paths.Select(path => Path.Combine(path, $"{e.Name.KebabCase}-name-set.enum.gen.ts")));
            ...
            foreach (var dto in project.TsDto)
            {
                ...
                foreach (var model in dto.Models)
                {
                    foreach (var property in model.TsProperties)
                    {
                        ...
                        else if (property.TypeNameType == TypeNameEnum.Enum && property.Options.JsonIgnore != JsonIgnoreCondition.Always)
                        {
