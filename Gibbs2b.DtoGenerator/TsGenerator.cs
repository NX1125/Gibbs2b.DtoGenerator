    ...
    public void Generate()
    {
        ...
        if (enumTypeScriptProject != null)
        {
            foreach (var e in _solution.Project.Enums.Values.OrderBy(p => p.Name.CapitalCase))
            {
                ...
                e.TsArrayPath = Path.Combine(enumTypeScriptProject.DefaultEnumPath!, $"{e.DtoName.KebabCase}-names.enum.gen.ts");
        ...
        _logger.LogInformation("Collecting imports");
        {
            ...
            foreach (var dto in project.TsDto.Values)
            {
                ...
                var absoluteFrom = Path.Combine(dto.TsProject!.EnumPathPrefix ?? dto.TsProject.DefaultDtoPath, $"{dto.DtoName.KebabCase}.dto.gen");
